...
import {
  ...
  ChevronDown, CreditCard, LayoutGrid, MessageCircle, ShieldCheck, Database
} from 'lucide-react';
...
export default function AppFrame({
  ...
  const avatarUrl = user?.avatar_url || `https://ui-avatars.com/api/?name=${encodeURIComponent(user?.full_name || 'User')}&background=0052FF&color=fff&bold=true&size=160`;
  ...
  return (
    <div className="flex min-h-screen bg-[#FDFDFF] relative selection:bg-blue-100 selection:text-blue-900 overflow-x-hidden font-opensans antialiased">
      <div className="fixed inset-0 z-0 overflow-hidden pointer-events-none">
        <div className="absolute top-[-5%] left-[-5%] w-[40%] h-[40%] rounded-full bg-blue-500/[0.04] blur-[120px]" />
        <div className="absolute bottom-[-5%] right-[-5%] w-[40%] h-[40%] rounded-full bg-indigo-500/[0.04] blur-[120px]" />
      </div>
      <div className="hidden md:block">
        <Sidebar activeTab={activeTab} setActiveTab={setActiveTab} isCollapsed={isCollapsed} toggleSidebar={() => setIsCollapsed(!isCollapsed)} lang={lang} />
      </div>
      <main className={`flex-1 relative transition-all duration-700 ease-in-out z-10 flex flex-col ${isCollapsed ? 'md:ml-20' : 'md:ml-64'} mb-20 md:mb-0`}>
        <header className="sticky top-0 z-[100] bg-white/80 backdrop-blur-3xl border-b border-slate-200/40 px-6 md:px-10 py-5 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3 md:gap-6 font-ui">
            ...
            <div className="hidden lg:flex items-center bg-slate-100/40 rounded-sm px-4 py-2.5 border border-slate-200/50 w-64 focus-within:w-80 transition-all focus-within:bg-white focus-within:ring-8 focus-within:ring-blue-500/5">
              ...
              <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder={t.search_placeholder} className="bg-transparent border-none outline-none text-xs font-bold ml-2 w-full text-slate-700" />
            </div>
            <div className="flex items-center gap-2">
              <button onClick={() => openMessenger('support')} className="p-2.5 text-slate-400 hover:text-[#0052FF] transition-all relative">
            ...
            <div className="w-px h-8 bg-slate-200/50 mx-2 hidden sm:block" />
            <div className="relative">
              <button onClick={() => setIsProfileMenuOpen(!isProfileMenuOpen)} className="flex items-center gap-3 p-1 rounded-full hover:bg-slate-50 transition-all border border-transparent hover:border-slate-100">
                ...
                <>
                  ...
                  <div className="absolute right-0 mt-3 w-72 bg-white rounded-sm shadow-2xl border border-slate-100 py-4 z-20 animate-scale-up">
                    <div className="px-6 py-4 border-b border-slate-50">
                      ...
                      <h4 className="font-black text-slate-900 truncate">{user?.full_name || 'Loading...'}</h4>
                    ...
                    <div className="py-2">
                      {[
                        ...
                        { label: 'Account Settings', icon: <CreditCard size={14} />, action: () => setActiveTab('account') },
                        { label: 'System Diagnostics', icon: <Database size={14} />, action: () => setActiveTab('reports') },
                      ].map((item, i) => (
                        <button key={i} onClick={() => { item.action?.(); setIsProfileMenuOpen(false); }} className="w-full flex items-center gap-3 px-6 py-3 text-slate-500 hover:text-blue-600 hover:bg-slate-50 transition-all">
        ...
        <div className={`flex-1 animate-fade-in ${isDashboard ? 'p-0' : 'p-6 md:p-10'}`}>{children}</div>
        <AiAssistant isOpen={isAiOpen} onClose={() => setIsAiOpen(false)} lang={lang} />
        <MessengerOverlay isOpen={isMessengerOpen} onClose={() => setIsMessengerOpen(false)} context={messengerContext} currentUser={user} conversation={null} />
      </main>
      <nav className="md:hidden fixed bottom-6 left-1/2 -translate-x-1/2 z-[100] w-[90%] h-18 bg-white/80 backdrop-blur-2xl border border-slate-200/50 rounded-sm shadow-2xl flex items-center justify-around px-4">
        {[{ id: 'dashboard', icon: <LayoutGrid size={22} />, label: 'Nexus' }, { id: 'inventory', icon: <ShoppingBag size={22} />, label: 'Assets' }, { id: 'movements', icon: <Search size={22} />, label: 'Flow' }].map(btn => (
